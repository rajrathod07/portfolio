...
import { Code2, ShieldCheck, Terminal, Cpu, Activity } from 'lucide-react';
...
export default function Skills() {
  return (
    <section id="skills" className="relative w-full max-w-[1300px] mx-auto px-6 md:px-10 py-16 md:py-24 select-none z-0">
      ...
      {/* SECTION HEADER - Placed on z-[30] to stay above cursor */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {skillCategories.map((category, index) => (
          ...
          >
            {/* CONTENT WRAPPER - z-[30]: The "Ceiling" (Spotlight slides between this and the card floor) */}
                ...
                {category.skills.map((skill, sIdx) => (
                  <div key={sIdx} className="w-full">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[#000] text-[10px] font-black uppercase tracking-wider">
                        {skill.name}
                      </span>
      ...
      {/* BACKGROUND DECORATION - Lower z-index so cursor passes over it */}
